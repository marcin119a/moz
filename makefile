...
#   1.10 : 2007.09.09 : AI - na zyczenie Jakuba Radoszewskiego weryfikator nie
#   												 jest uruchamiany jesli program zakonczy sie beledem
#   1.11 : 2008.01.29 : AI - wymuszanie aby program w Javie dawal poprawne
#   												 odpowiedzi w przeznaczonym czasie i pamieci
#   1.12 : 2008.10.17 : AI - dodanie do raportu dodatkowych informacji o systemie
#                            (pamiec, obciazenie procesora, limity)
#   1.13 : 2009.01.22 : AI - usuniecie ulimita dla Javy
#   1.14 : 2009.01.29 : AI - poprawienie porownania miedzy stringami przy checkerce
#   1.15 : 2009.02.03 : AI - dodanie mozliwosci ustawienia CFLAGS i swojego makefile'a
#   1.16 : 2009.03.27 : AI - dodanie pomijania mpstat (jesli nie ma go w systemie) oraz
#                            mozliwosci quickver (weryfikacja z pominieciem raportu)
#   1.17 : 2009.04.17 : AI - dodanie wymuszanie przebudowania dokumentow (.ps i .pdf),
#                            tak, aby miec pewnosc ze kompiluja sie na SInOlu.
#   1.18 : 2009.05.06 : AI - dodanie testowania valgrindem rozwiazania wzorcowego.
#   1.19 : 2009.12.05 : AI - dodanie usuwania *.valgrind do make clean w katalogu prog.
#   1.20 : 2009.12.09 : AI - dodanie zapezpiecznia przed zbyt dlugim dzialaniem programu.
#   1.21 : 2009.12.12 : AI - dodanie weryfikowania sum kontrolnych danych wejsciowych
#   1.22 : 2009.12.27 : AI - uruchomienie sleep jako oddzielnej grupy i zabicie jej.
#   1.23 : 2010.03.10 : AI - wiecej czasu rzeczywistego dla procesow, to tylko zabezpieczenie
#   1.24 : 2010.03.12 : AI - naprawa balaganu po real timeout, poprawa kolorow w raporcie,
#                            wyglada na to ze czasem sleep startuje pozniej i kill go nie zabija.
...
B_PROGS_WER= $(shell find prog -regex "prog/$(ID)[0-9]*wer[0-9]*\.\(c\|cpp\|pas\|ml\|java\)" | sort -n -k 1.`echo "prog/$(ID)"| wc -c`)
B_PROGS_SLW= $(shell find prog -regex "prog/$(ID)[0-9]*s\.\(c\|cpp\|pas\|ml\|java\)" | sort -n -k 1.`echo "prog/$(ID)"| wc -c`)
B_PROGS_BAD= $(shell find prog -regex "prog/$(ID)[0-9]*b\.\(c\|cpp\|pas\|ml\|java\)" | sort -n -k 1.`echo "prog/$(ID)"| wc -c`)
...
######################################################################
# Usuni�cie test�w genereowanych automatycznie
######################################################################
clean-gen:
	@if [ "$(INGEN)" -eq 1 ] ; then \
...
############################################################################
# Weryfikacja losowosci danych wej�ciowych
############################################################################
ingen_md5: $(MD5SUMS)
...
inwer:
	@if [ "$(INWER)" -eq 1 ] ; then \
		...
		echo "Brak weryfikatora do test�w" ; \
		exit 2 ; \
	fi
	@md5sum in/* > $(MD5SUMS)
...
############################################################################
# Wygenerowanie wzorcowych odpowiedzi dla bie��cego zestawu test�w
############################################################################
outgen:
	make -C prog $(ID).e
	@for i in $(TESTS) ; do \
...
############################################################################
# Uruchomienie programu prog/$(ID)%.e na bie��cym zestawie test�w
